...
class FindCorrectParser():
    ...
    def find_parser(self, file_path: str) -> List[Transaction]:
            ...
            for row in table_data:
                ...
                for item in row:
                    if isinstance(item, str):   
                        ...
                        formatted_all_parsers = {perm_key:value for key, value in all_parsers.items() for perm_key in self._get_all_versions_of_key(key)}
                        ...
                        for parser_key in formatted_all_parsers:
                            if parser_key in formatted_item:
                                self.logger.info(f"Found correct parser: {formatted_all_parsers[parser_key]}")
                                return formatted_all_parsers[parser_key]
        raise  CorrectParserNotFound()
    def _format_string(self, string: str) -> str:
        text = string.lower()
        text = re.sub(r"[_\-]+", " ", text) # Replace underscores, hyphens, and other special characters with spaces
        text = re.sub(r"[^\w\s]", " ", text) # Remove any non-alphanumeric characters except spaces
        text = re.sub(r"\s+", " ", text).strip() # Optional: Collapse multiple spaces into one
        return text
